...
```{r, results='hide'}
# Let's prepare data
yelp <- read_rds(here("Lab", "module_1", "week4", "yelp_in.rds"))
# Census data
...
leaflet() %>% 
  ...
  addControl(title, className="map_title")
```
...
fig2 <- ggplot(data = census_yelp) +
  geom_point(mapping = aes(x=review_count, y=rating, alpha=price, 
                           size=price)) #<<
...
Remember that if you use `facet_wrap` with a continuous data, it will generate as many plots as the unique values in the continuous data.
