...
get_yelp_poss <- purrr::possibly(.f = get_yelp, otherwise = NULL)

# Looping through all Census Tracts
yelp_all_list <- map(1:nrow(ready_4_yelp), ~{ get_yelp_poss(ready_4_yelp[.x,], "restaurants"); Sys.sleep(5) } )

# Collapsing the list into a data.frame
yelp_all <- yelp_all_list %>% bind_rows() %>% as_tibble()

# Saving the downloaded data
yelp_all %>% write_rds(here("Lab", "module_1", "week1", "yelp_all_restaurants.rds"))
