...
In this document, we will use API(application programming interface) to acquire data from the Census Bureau & from Yelp Fusion API. According to Wiki, "An API is a way for two or more computer programs to communicate with each other." To use API, we generally need to connect to the API's endpoints that usually looks something like `https://api.yelp.com/v3`. We then append other parameters to complete the call. For example, the following is requesting **business search** service using a keyword **delis** for the location latitude of 37.787 and longitude of -122.399.

> `https://api.yelp.com/v3/businesses/search?term=delis&latitude=37.786882&longitude=-122.399972`

We would use e.g., `GET()` function in **httr package** to send API request to the server, and the server would return something like the following to the authenticated requests.
...
```{r}
new_functions <- data.frame(
  f = c("tidycensus::get_acs", "sf::st_bbox", "sf::st_point", "sf::st_centroid", "sf::st_distance", "sf::st_geometry", "sf::st_transform", "sf::st_coordinates", "sf::st_set_geometry", "sf::st_as_sf", "sf::st_join", "sapply", "min", "max", "abs", "seq", "expand.grid", "names", "paste0", "ceiling", "ifelse", "bind_rows", "Sys.getenv", "list.files", "units::set_units", "osmdata::getbb", "yelpr::business_search", "jsonlite::flatten", "purrr::map_lgl", "reshape2::melt"),
  d = c("Download ACS data", "Get the bounding box", "Create POINT")
)
```
