...
```{r}
net <- sfnetworks::as_sfnetwork(osm_road$osm_lines)
print(net)
...
subdiv_map <- leaflet() %>% 
  addProviderTiles(providers$CartoDB.DarkMatter) %>% 
  setView(-84.36508925130082, 33.77122894799306, zoom = 14) %>% # zooming in to show more details
...
smooth_map <- leaflet() %>% 
  ...
  addPolylines(data = smoothed_net %>% st_as_sf('edges'), col = "grey", weight = 3, opacity = 0.9) %>% 
    ...
    addCircles(data = subdiv_net %>% st_as_sf("nodes") %>% filter(custom_id %in% removed), 
               fillColor = "red", stroke = F, radius = 30, fillOpacity = 0.3)
...
network_char <- contracted %>% 
  ...
  mutate(bc = centrality_betweenness(weights = weight, directed = FALSE),
         bc_scale = ((min(bc) - bc)/(max(bc)-min(bc)))*20)

bet_pal <- colorQuantile(palette = "Reds", domain = network_char %>% activate("nodes") %>% pull(bc), n = 10)

leaflet() %>% 
  addProviderTiles(providers$CartoDB.DarkMatter) %>%
  setView(-84.36508925130082, 33.77122894799306, zoom = 14) %>% # zooming in to show more details
  addPolylines(data = contracted %>% st_as_sf("edges"), color = "grey", weight = 2) %>% 
  addCircles(data = network_char %>% st_as_sf("nodes"), fillColor = ~bet_pal(bc), fillOpacity = 0.9, stroke = F, 
             radius = network_char %>% st_as_sf('nodes') %>% pull(bc_scale))
...
leaflet() %>% 
  ...
  setView(-84.36508925130082, 33.77122894799306, zoom = 14) %>% # zooming in to show more details
  addPolylines(data = smoothed_net %>% st_as_sf('edges'), col = "grey", weight = 3, opacity = 0.9) %>% 
    addCircles(data = intersections %>% st_as_sf('nodes'), fillColor = 'skyblue', stroke = F, radius = 20, fillOpacity = 0.7) 
```
...
<!-- ```{r eval=F} -->
<!-- # get OSM -->
<!-- net <- dodgr_streetnet(my_bb) -->

<!-- osm_net <- opq(bbox = my_bb) %>% -->
<!--   add_osm_feature(key = 'highway', -->
