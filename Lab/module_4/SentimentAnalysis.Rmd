...
3. Go to Tools > Global Options > Python  and 
4. Load the package using `library(sentiment.ai)`.
5. Run the following code. It will install a lot of stuff and will restart R session at the end. Next time you open RStudio, you don't need to run this code again.
...
6. After R session is restarted, run `check_sentiment.ai()`. You will be asked if you wanted to install Miniconda. Hit y. This Miniconda is different from the one you've already installed because this one is for the use within R. This function may print some error messages, but that's fine as long as the final message says NULL. 

7. Run the following code for testing purpose.

```{r}
# Next time you open RStudio, all you need to do is to
# library(sentiment.ai) and init_sentiment.ai().
check_sentiment.ai()
